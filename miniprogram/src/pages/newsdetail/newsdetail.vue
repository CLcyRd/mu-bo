    ...
    <scroll-view class="detail-scroll" scroll-y>
      ...
      <view v-else>
        <view class="article-cover-wrapper">
          <image class="article-cover" :src="getCoverSrc()" mode="aspectFill" @error="handleCoverError" />
...
const getCoverSrc = () => {
  if (failedCover.value) {
    return fallbackCover
  }
  const cover = resolveCoverUrl(article.value.cover)
  return cover || fallbackCover
}

const handleCoverError = () => {
  failedCover.value = true
}
...
onShareAppMessage(() => {
  return {
    ...
    path: `/pages/newsdetail/newsdetail?id=${article.value.id}`,
